...
execute as @a[nbt={Inventory: [{Slot:100b, tag:{relic:"solomons_shoes"}}]},scores={playerSneak=1}] at @s if block ~ ~-1 ~ air run scoreboard players add @s solomonsShoes 1
...
execute as @a[scores={solomonsShoes=1}] at @s run summon minecraft:armor_stand ~ ~-1 ~ {Tags: ["solomonsShoes"], Marker: true, NoGravity: true, NoAI: true, Invisible: true, Invulnerable: true, Silent: true, Small: true, Passengers:[{id: shulker, Tags: ["solomonsShoes"], NoAI: true, NoGravity: true, Silent: true, DeathLootTable: "minecraft:empty", Invulnerable: true, DeathTime: 19, ActiveEffects: [{Id: 14, Amplifier: 1, Duration: 999999, ShowParticles: false}]}, {id: falling_block, Tags: ["solomonsShoes"], BlockState: {Name: "minecraft:white_stained_glass"}, Time: 1, DropItem: false, NoGravity: true}]}
...
execute as @e[type=shulker,tag=solomonsShoes,scores={solomonsShoes=1}] at @s run playsound minecraft:ui.toast.in player @a ~ ~ ~ 1.4 1.4
