...
execute as @s[scores={witherInvul=1..100}] unless entity @e[tag=wither_fallout,distance=..5] run summon minecraft:area_effect_cloud ~ ~ ~ {Tags:["wither_fallout","wither_summon"],Color:7047818,Duration:24000,Particle:"minecraft:smoke",Radius:5,Effects:[{Id:20,Duration:800,Amplifier:1}]}

# increment powerCooldown if health is below half
