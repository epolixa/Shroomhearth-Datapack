...
execute unless predicate community:has_charm run tellraw @s {"translate":"community.missing_charm","hoverEvent":{"action":"show_text","contents":{"translate":"community.missing_charm_tooltip"}}}

# If the player has charm, activate
execute if predicate community:has_charm run function community:favor/disable_raids/activate