...
$tellraw @a[tag=debug_echo_horn] [{"text":"[echo_horn:m_teleport_to_respawn] Teleporting "},{"selector":"@s"},{"text":" to their respawn point: "},{"text":"$(respawn_x), $(respawn_y), $(respawn_z) in $(respawn_dimension)"}]
...
$execute in $(respawn_dimension) unless block $(respawn_x) $(respawn_y) $(respawn_z) #echo_horn:respawn_blocks run function echo_horn:teleport_to_world_spawn