...
tellraw @a[tag=debug_echo_horn] [{"text":"[echo_horn:teleport_to_player] Target player "},{"selector":"@s"},{"text":" found for "},{"selector":"@p[tag=checking_teleport_target]"}]


# Mark teleportation as successful for the echo horn user
tag @p[tag=checking_teleport_target] add echo_horn_teleport_success
