...
tellraw @a[tag=debug_harmony] [{"text":"[harmony:spores/haste/activate] "},{"translate":"harmony.spores.haste"},{"text":" activated by "},{"selector":"@s"}]

# Update active spores if not already active
execute if score spores_of_haste shroomhearth matches 0 run scoreboard players add active_spores shroomhearth 1
