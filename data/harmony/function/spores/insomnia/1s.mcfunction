...
tellraw @a[tag=debug_harmony] [{"text":"[harmony:spores/insomnia/1s] processing "},{"translate":"harmony.spores.insomnia"}]
...
execute if score spores_of_insomnia shroomhearth.harmony matches ..0 run function harmony:spores/insomnia/deactivate

# Chance to play particle effect around players
execute as @a at @s run execute if predicate harmony:spores_particle_chance run particle minecraft:dust{color:[0.31, 0.38, 0.64], scale: 1} ~ ~1 ~ 10 10 10 0.01 1 force
