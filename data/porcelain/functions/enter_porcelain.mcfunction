...
execute at @s run summon minecraft:armor_stand ~ ~-1.9 ~ {CustomName:'{"text":"Overworld Anchor"}', Tags: ["overworld_anchor"], Marker: true, Invisible: true, ArmorItems:[{},{},{},{id:"minecraft:warped_fungus_on_a_stick", Count:1b, tag: {CustomModelData: 1, Enchantments:[{id:"minecraft:infinity",lvl:1}]}}]}

# copy rotation of player to nearest anchor in overworld
data modify entity @e[type=minecraft:armor_stand,tag=porcelain_anchor,sort=nearest,limit=1] Rotation[0] set from entity @s Rotation[0]
data modify entity @e[type=minecraft:armor_stand,tag=porcelain_anchor,sort=nearest,limit=1] Rotation[2] set from entity @s Rotation[2]
