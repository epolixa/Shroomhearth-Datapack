...
tellraw @a[tag=debug_echo_horn] [{"text":"[shroomhearth:echo_horn/check_respawn_point] Checking respawn point for player "},{"selector":"@s"}]
...
at @s if block ~ ~ ~ #shroomhearth:respawn_blocks \
run function shroomhearth:echo_horn/found_respawn_block
tag @s remove checking_respawn_point

# Log whether a valid respawn block was found
tellraw @a[tag=debug_echo_horn,tag=!has_respawn_block] [{"text":"[shroomhearth:echo_horn/check_respawn_point] No respawn block found at respawn point of "},{"selector":"@p[tag=using_own_echo_horn]"}]