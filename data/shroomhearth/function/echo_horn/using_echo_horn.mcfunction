...
tellraw @a[tag=debug_echo_horn] [{"text":"[shroomhearth:echo_horn/using_echo_horn] "},{"selector":"@s"},{"text":" using Echo Horn"}]

# Interrupt cast if not holding echo horn
execute unless predicate shroomhearth:echo_horn/holding_echo_horn run function shroomhearth:echo_horn/advancement_reward/echo_horn_interrupted
...
execute as @s[tag=using_own_echo_horn,tag=has_respawn_data,tag=!has_respawn_block] run function shroomhearth:echo_horn/check_respawn_point

# Initiate the teleport once the cast time exceeds 5 seconds
execute if score @s using_echo_horn matches 5.. run function shroomhearth:echo_horn/teleport
