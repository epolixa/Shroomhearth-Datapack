...
execute as @e[tag=harmony_cap_marker] at @s if block ~ ~-0.0625 ~ #shroomhearth:harmony/harmony_cap_grows_in_with_layers[layers=1] run tp @s ~ ~-1 ~
execute as @e[tag=harmony_cap_marker] at @s if block ~ ~-0.0625 ~ #shroomhearth:harmony/harmony_cap_grows_in run tp @s ~ ~-1 ~

# Check if the marker can spawn a harmony cap
execute as @e[tag=harmony_cap_marker] at @s run function shroomhearth:harmony/harmony_cap/check_marker
