...
execute at @s positioned ~ ~-0.0625 ~ if predicate shroomhearth:harmony/harmony_cap_can_grow_one_block_into at @s run tp @s ~ ~-1 ~

# Move position one texel down if spreadplayers landed on top of a dirt path
execute at @s if block ~ ~-0.0625 ~ #shroomhearth:harmony/harmony_cap_grows_one_texel_into run tp @s ~ ~-0.0625 ~
