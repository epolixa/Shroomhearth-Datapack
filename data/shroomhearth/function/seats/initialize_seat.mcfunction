# Executor: A seat interaction entity that has been checked and is being initialized.
# Position: The seat interaction entity.

tellraw @a[tag=debug_seats] [{"text":"[shroomhearth:seats/initialize_seat] Initializing seat"}]
...
execute on passengers as @p[tag=placed_seat] at @n[tag=seat_display,tag=!initialized] run function shroomhearth:seats/set_seat_rotation
