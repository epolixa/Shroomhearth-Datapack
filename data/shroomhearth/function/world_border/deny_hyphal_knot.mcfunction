...
tellraw @a[tag=debug_world_border] [{"text":"[shroomhearth:world_border/deny_hyphal_knot] "},{"selector":"@s"},{"text":" failed to unravel a Hyphal Knot while the World Border is growing"}]
...
execute as @s[gamemode=survival] run loot spawn ~ ~ ~ loot shroomhearth:world_border/hyphal_knot
